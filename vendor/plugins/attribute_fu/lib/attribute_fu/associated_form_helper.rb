  ...
  module AssociatedFormHelper
    ...
    def add_associated_link(name, object, opts = {})
      ...
      @template.link_to_function(name, opts) do |page|
        ...
        page << "new Insertion.Bottom(#{container}, new Template("+form_builder.render_associated_form(object, opts.merge(:fields_for => { :javascript => true }, :partial => partial)).to_json+").evaluate({'number': --#{variable}}).gsub(/__number_/, #{variable}))"
      end
    end
