...
for(var i=0;i<width;i++){var w=Math.max(0,getWidth(fx,i,width));var e=strip.cloneNode(false);e.style.borderWidth='0 '+(opts[j+'R']?w:0)+'px 0 '+(opts[j+'L']?w:0)+'px';bot?d.appendChild(e):d.insertBefore(e,d.firstChild);}}}});};$.fn.uncorner=function(){$('div.jquery-corner',this).remove();return this;};})(jQuery);var confirmUnload=false;function setConfirmUnload(on){confirmUnload=on;}
$(document).ready(function(){if($('#exhibition').length==0)
return;$('form :input').bind("change",function(){confirmUnload=true;});window.onbeforeunload=winClose;});function winClose(evt){if(confirmUnload){var message="There are some unsaved changes. If you leave this page they will be lost!";if(typeof evt==undefined)
evt=window.event;if(evt)
evt.returnValue=message;return message;}
confirmUnload=false;};(function($){var expr=(function(){if(!$.browser.msie)return false;var div=document.createElement('div');try{div.style.setExpression('width','0+0');}
catch(e){return false;}
return true;})();function sz(el,p){return parseInt($.css(el,p))||0;};function hex2(s){var s=parseInt(s).toString(16);return(s.length<2)?'0'+s:s;};function gpc(node){for(;node&&node.nodeName.toLowerCase()!='html';node=node.parentNode){var v=$.css(node,'backgroundColor');if(v=='rgba(0, 0, 0, 0)')
continue;if(v.indexOf('rgb')>=0){var rgb=v.match(/\d+/g);return'#'+hex2(rgb[0])+hex2(rgb[1])+hex2(rgb[2]);}
if(v&&v!='transparent')
...
for(var i=0;i<width;i++){var w=Math.max(0,getWidth(fx,i,width));var e=strip.cloneNode(false);e.style.borderWidth='0 '+(opts[j+'R']?w:0)+'px 0 '+(opts[j+'L']?w:0)+'px';bot?d.appendChild(e):d.insertBefore(e,d.firstChild);}}}});};$.fn.uncorner=function(){$('div.jquery-corner',this).remove();return this;};})(jQuery);function changeCount(count,dif){if(typeof($[count])=="undefined")
$[count]=0;$[count]+=dif;$('span#'+count).text($[count]);}
function replace_ids(s,parent){if(parent!=null){var parent_id=parent.find('input')[0].name.match(/.*\[(\d+)\]/)[1];s=s.replace(/PARENT_ID/g,parent_id);}
var new_id=new Date().getTime();return s.replace(/NEW_RECORD/g,new_id);}
function add_new(text,container,parent){var new_element=$(replace_ids(text,parent));new_element.hide().addClass('loading').fadeIn('fast',function(){$(this).removeClass('loading').find('input').bind('change',function(){setConfirmUnload(true);});});container.append(new_element);}
function remove(container){container.fadeOut('fast');container.find('input[type=hidden]').attr('value',1);setConfirmUnload(true);}
$(function(){$('table.sortable tr:even').addClass('even');$('table.sortable tr:odd').addClass('odd');var input=$('input').not('input[type=hidden]').get(0);if(input!=null)
input.focus();if($.browser.mozilla)
